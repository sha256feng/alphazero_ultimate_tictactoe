...
class UCTNode():
    def __init__(self, game, move, parent=None):
        ...
        self.is_expanded = False # whether the state has children, there is a func to set this flag;
        self.parent = parent # parent should also be a UCTNode; or DummyNode;
        self.children = {} # a dictionary of children nodes (board states)
        self.child_priors = np.zeros([7], dtype=np.float32)
        self.child_total_value = np.zeros([7], dtype=np.float32) # total value of a child, not sure why it is 7-dim
        self.child_number_visits = np.zeros([7], dtype=np.float32) # number of visits to child, why 7-dim?
        self.action_idxes = [] # available actions to that state of the game
...
def run_MCTS(args, start_idx=0, iteration=0):
    ...
    # Handling of multi-process MCTS
    if args.MCTS_num_processes > 1:
        ...
        net.eval()
        # Find the trained net file and load the parameters
        # if path exists, load the model; if not, initialize and start with that;
        current_net_filename = os.path.join("./model_data/",\
                                        net_to_play)
        if os.path.isfile(current_net_filename):
            checkpoint = torch.load(current_net_filename)
        ...
        # Get MC training drills for later training of the network
        with torch.no_grad():
